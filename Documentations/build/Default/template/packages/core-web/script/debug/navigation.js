        ...
        (function (Features) {
            ...
            var SearchDocumentFeature = (function (_super) {
                ...
                SearchDocumentFeature.prototype.initializeContent = function (rootSelector, isInitialLoad) {
                    ...
                    if ($("#i-search-container", this._rootSelector).length > 0) {
                        ...
                        $("#i-search-container", this._rootSelector)
                            ...
                            event.preventDefault();
                            _this.showSearchResult($(event.target).attr("href"));
                        });
                    }
                };
                ...
                SearchDocumentFeature.prototype.buildSearchResults = function () {
                    ...
                    for (var tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
                        if (tokens[tokenIndex].length > 0) {
                            ...
                            if ((token == "or") || (token == "and")) {
                                ...
                                // add to query string
                                query += ("(keyword.indexOf(\"" + token + "\") >= 0)");
                                lastTokenWasOperator = false;
                                lastTokenWasKeyword = true;
                                // add to keywords array
                                keywords[keywordIndex] = token;
                                keywordIndex++;
                            }
                        }
                    }
                    ...
                    var indexFileTitles = [];
                    var indexStopWords = [];
                    Innovasys.Content.Features.buildSearchIndexArray(indexFiles, indexFileTitles, indexStopWords);
                    // build list of files containing keywords
                    var files = [];
                    var fileTitles = [];
                    var fileKeywords = [];
                    this.loadFilesWithKeywords(indexFiles, indexFileTitles, indexStopWords, keywords, files, fileTitles, fileKeywords);
                    // build results list
                    var results = this.buildResultsArray(query, fileKeywords);
                    // write results to document
                    this.outputResults(results, files, fileTitles);
                    // return success
                    return true;
                };
                SearchDocumentFeature.prototype.loadFilesWithKeywords = function (indexFiles, indexFileTitles, indexStopWords, keywords, files, fileTitles, fileKeywords) {
                    // build lookup arrays
                    for (var stopWordIndex = 0; stopWordIndex < keywords.length; stopWordIndex++) {
                        // find stop word in index
                        if (indexStopWords["_" + keywords[stopWordIndex]] != undefined) {
                            // get indices of files containing word
                            var indexStopWord = indexStopWords["_" + keywords[stopWordIndex]];
                            // tokenize indices
                            var fileIndexes = indexStopWord.split(",");
                            for (var index = 0; index < fileIndexes.length; index++) {
                                // add to array
                                var fileIndex = parseInt(fileIndexes[index]);
                                if (fileKeywords[fileIndex] == undefined) {
                                    fileKeywords[fileIndex] = keywords[stopWordIndex];
                                }
                                else {
                                    fileKeywords[fileIndex] += ("," + keywords[stopWordIndex]);
                                }
                    ...
                    for (var fileNameIndex = 0; fileNameIndex < fileKeywords.length; fileNameIndex++) {
                        if (fileKeywords[fileNameIndex] != undefined) {
                            // find stop word in index
                            if (indexFiles[fileNameIndex] != undefined) {
                                files[fileNameIndex] = indexFiles[fileNameIndex];
                                fileTitles[fileNameIndex] = indexFileTitles[fileNameIndex];
                            }
                        }
                    }
                };
                SearchDocumentFeature.prototype.buildResultsArray = function (query, fileKeywords) {
                    var results = [];
                    for (var index = 0; index < fileKeywords.length; index++) {
                        if (fileKeywords[index] != undefined) {
                            var keyword = fileKeywords[index];
                            var result = false;
                            try {
                                result = eval(query);
                            }
                ...
                SearchDocumentFeature.prototype.outputResults = function (results, files, fileTitles) {
                    // init table html
                    ...
                    for (var resultIndex = 0; resultIndex < results.length; resultIndex++) {
                        ...
                        if (results[resultIndex] != undefined) {
                            ...
                            tableHtml += "<tr><td class=\"" + className + "\">"
                                + (resultCount + 1) + ":</td><td class=\"" + className + "\"><a class=\"i-result btn\" target=\"i-content\" href=\"" + files[results[resultIndex]] + "\">"
                                + fileTitles[results[resultIndex]] + "</a></td></tr>";
                            // incr total
                            resultCount++;
                        }
                    }
                ...
                SearchDocumentFeature.prototype.showSearchResult = function (hRef) {
                ...
                SearchDocumentFeature.prototype.highlightContentFrame = function () {
                    var keywords = this.getKeywordArray();
